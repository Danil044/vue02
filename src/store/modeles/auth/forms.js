export default {
  ...
  mutations: {
    ...
    passwordConfirm: (state, data) => { state.passwordConfirm = data },
  ...
  actions: {
    ...
    passwordConfirm ({ commit, dispatch }, data) {
      if (!data) data = ''

      commit('passwordConfirm', data)
      dispatch('validateForm')
    },
    ...
    validateForm ({ state, commit, dispatch }, data = null) {
      ...
      if (email.length < 2) {
        commit('errLogin', 'short')
      }
      if (password.length < 2) {
        commit('errPassword', 'short')
      }
